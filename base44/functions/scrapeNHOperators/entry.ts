...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main NH page to get all cities dynamically
    console.log('Fetching main NH page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/NH');
    const mainPageHtml = await mainPageResponse.text();
    
    // Extract all city links
    const cityLinkRegex = /href="(\/companies\/NH\/[^"]+)"/g;
    ...
    console.log(`Found ${cityLinks.length} NH cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/NH\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/NH\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'NH' }, '-created_date', 2000);
    ...
    console.log(`Total NH Estate Sale Company Owners in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in NH`);
