...
Deno.serve(async (req) => {
  try {
    ...
    const result = await base44.asServiceRole.functions.invoke(
      "closeDeal",
      { deal_id, actual_commission, shared_key: HOUSZU_SHARED_KEY },
      { appId: HOUSZU_APP_ID }
    );

    console.log(`[notifyHouszu_CloseDeal] deal=${deal_id} commission=${actual_commission} referral=${result?.referral_amount_due_to_brent}`);
