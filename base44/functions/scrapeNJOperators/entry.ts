...
Deno.serve(async (req) => {
  try {
    ...
    // batch_offset: which company index to start saving from (0 = first batch)
    // all_companies: if provided, skip re-scraping and use this cached list
    ...
    if (cachedCompanies) {
      // Use the cached list from the frontend to avoid re-scraping
      allCompanies = cachedCompanies;
      console.log(`Using cached list of ${allCompanies.length} companies, offset=${batchOffset}`);
    } else {
      // Scrape all cities fresh
      console.log('Fetching main NJ page to extract cities...');
      const mainPageResponse = await fetch('https://www.estatesales.net/companies/NJ');
      const mainPageHtml = await mainPageResponse.text();
      
      const cityLinkRegex = /href="(\/companies\/NJ\/[^"]+)"/g;
      const cityLinks = [...mainPageHtml.matchAll(cityLinkRegex)]
        .map(m => `https://www.estatesales.net${m[1]}`)
        .filter((url, idx, arr) => arr.indexOf(url) === idx);
      
      console.log(`Found ${cityLinks.length} NJ cities to process`);
      ...
      for (const cityUrl of cityLinks) {
        try {
          ...
          for (let i = 1; i < companyBlocks.length; i++) {
            ...
            try {
              let nameMatch = block.match(/href="\/companies\/NJ\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
              if (!nameMatch) nameMatch = block.match(/itemprop="name"[^>]*>([^<]+)</);
              const name = nameMatch ? nameMatch[1].trim() : null;
              
              const urlMatch = block.match(/href="(\/companies\/NJ\/[^"]+\/\d{5}\/\d+)"/);
    ...
    // Load ALL existing NJ records for lookup (paginated)
    let existing = [];
    let skip = 0;
    while (true) {
      const page = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'NJ' }, '-created_date', 500, skip);
    ...
    return Response.json({
      ...
      all_companies: allCompanies, // return full list so frontend can cache it
