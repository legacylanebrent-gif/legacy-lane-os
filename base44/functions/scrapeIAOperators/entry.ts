...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main IA page to get all cities dynamically
    console.log('Fetching main IA page to extract cities...');
    ...
    console.log(`Found ${cityLinks.length} IA cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/IA\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/IA\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'IA' }, '-created_date', 2000);
    ...
    console.log(`Total IA operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in IA`);
