...
Deno.serve(async (req) => {
  try {
    ...
    if (cachedCompanies) {
      ...
      console.log('Fetching main AR page to extract cities...');
      const mainPageResponse = await fetch('https://www.estatesales.net/companies/AR');
      ...
      console.log(`Found ${cityLinks.length} AR cities to process`);
      ...
      for (const cityUrl of cityLinks) {
        try {
          ...
          for (let i = 1; i < companyBlocks.length; i++) {
            ...
            try {
              let nameMatch = block.match(/href="\/companies\/AR\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
              if (!nameMatch) nameMatch = block.match(/itemprop="name"[^>]*>([^<]+)</);
              const name = nameMatch ? nameMatch[1].trim() : null;

              const urlMatch = block.match(/href="(\/companies\/AR\/[^"]+\/\d{5}\/\d+)"/);
    ...
    let skip = 0;
    while (true) {
      const page = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'AR' }, '-created_date', 500, skip);
