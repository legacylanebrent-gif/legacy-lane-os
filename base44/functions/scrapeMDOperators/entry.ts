...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main MD page to get all cities dynamically
    console.log('Fetching main MD page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/MD');
    const mainPageHtml = await mainPageResponse.text();
    
    // Extract all city links
    const cityLinkRegex = /href="(\/companies\/MD\/[^"]+)"/g;
    ...
    console.log(`Found ${cityLinks.length} MD cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/MD\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/MD\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'MD' }, '-created_date', 2000);
    ...
    console.log(`Found ${toDelete.length} duplicates in MD`);
