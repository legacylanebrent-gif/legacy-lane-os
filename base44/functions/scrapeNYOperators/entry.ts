...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main NY page to get all cities dynamically
    console.log('Fetching main NY page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/NY');
    ...
    console.log(`Found ${cityLinks.length} NY cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/NY\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/NY\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'NY' }, '-created_date', 2000);
    ...
    console.log(`Total NY Estate Sale Company Owners in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in NY`);
