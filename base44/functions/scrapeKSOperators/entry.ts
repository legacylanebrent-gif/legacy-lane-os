...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main KS page to get all cities dynamically
    console.log('Fetching main KS page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/KS');
    const mainPageHtml = await mainPageResponse.text();
    
    // Extract all city links
    const cityLinkRegex = /href="(\/companies\/KS\/[^"]+)"/g;
    ...
    console.log(`Found ${cityLinks.length} KS cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/KS\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/KS\/[^"]+\/\d{5}\/\d+)"/);
    ...
    console.log(`Total KS Estate Sale Company Owners in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in KS`);
