...
Deno.serve(async (req) => {
  try {
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'TX' }, '-created_date', 2000);
    ...
    console.log(`Total TX Estate Sale Company Owners in database: ${allOperators.length}`);
