...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main AL page to get all cities dynamically
    console.log('Fetching main AL page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/AL');
    const mainPageHtml = await mainPageResponse.text();
    
    // Extract all city links
    const cityLinkRegex = /href="(\/companies\/AL\/[^"]+)"/g;
    ...
    console.log(`Found ${cityLinks.length} AL cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/AL\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/AL\/[^"]+\/\d{5}\/\d+)"/);
    ...
    console.log(`Total AL operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in AL`);
