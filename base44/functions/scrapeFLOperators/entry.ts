...
Deno.serve(async (req) => {
  try {
    ...
    // Step 1: Scrape all cities (only on first call, when no cache)
    if (cachedCompanies) {
    ...
    // Step 2: Load existing records from DB (only on first call, cache for subsequent batches)
    let byUrl, byPhone;
    if (cachedExisting) {
      byUrl = new Map(cachedExisting.byUrl);
      byPhone = new Map(cachedExisting.byPhone);
    } else {
      let existing = [];
      let skip = 0;
      while (true) {
      ...
      byUrl = new Map(existing.filter(e => e.source_url).map(e => [e.source_url, { id: e.id }]));
      byPhone = new Map(existing.filter(e => e.phone).map(e => [e.phone, { id: e.id }]));
    }

    // Step 3: Save this batch
    ...
    const existingMapForNext = {
      byUrl: [...byUrl.entries()],
      byPhone: [...byPhone.entries()],
    };
