...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main FL page to get all cities dynamically
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'FL' }, '-created_date', 2000);
    ...
    console.log(`Total FL operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in FL`);
