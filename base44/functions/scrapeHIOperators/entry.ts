...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main HI page to get all cities dynamically
    console.log('Fetching main HI page to extract cities...');
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/HI\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/HI\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'HI' }, '-created_date', 2000);
    ...
    console.log(`Total HI operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in HI`);
