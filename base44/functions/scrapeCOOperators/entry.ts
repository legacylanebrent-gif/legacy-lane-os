...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main CO page to get all cities dynamically
    console.log('Fetching main CO page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/CO');
    ...
    console.log(`Found ${cityLinks.length} CO cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/CO\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/CO\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'CO' }, '-created_date', 2000);
    ...
    console.log(`Total CO operators in database: ${allOperators.length}`);
