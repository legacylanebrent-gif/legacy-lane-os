...
Deno.serve(async (req) => {
  try {
    ...
    if (cachedCompanies) {
      ...
      console.log('Fetching main DC page to extract cities...');
      const mainPageResponse = await fetch('https://www.estatesales.net/companies/DC');
      const mainPageHtml = await mainPageResponse.text();

      const cityLinkRegex = /href="(\/companies\/DC\/[^"]+)"/g;
      const cityLinks = [...mainPageHtml.matchAll(cityLinkRegex)]
        .map(m => `https://www.estatesales.net${m[1]}`)
        .filter((url, idx, arr) => arr.indexOf(url) === idx);

      console.log(`Found ${cityLinks.length} DC cities to process`);
      ...
      for (const cityUrl of cityLinks) {
        try {
          ...
          for (let i = 1; i < companyBlocks.length; i++) {
            ...
            try {
              let nameMatch = block.match(/href="\/companies\/DC\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
              if (!nameMatch) nameMatch = block.match(/itemprop="name"[^>]*>([^<]+)</);
              const name = nameMatch ? nameMatch[1].trim() : null;

              const urlMatch = block.match(/href="(\/companies\/DC\/[^"]+\/\d{5}\/\d+)"/);
    ...
    let skip = 0;
    while (true) {
      const page = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'DC' }, '-created_date', 500, skip);
