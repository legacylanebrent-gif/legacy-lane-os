...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main VT page to get all cities dynamically
    console.log('Fetching main VT page to extract cities...');
    ...
    console.log(`Found ${cityLinks.length} VT cities to process`);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'VT' }, '-created_date', 2000);
    ...
    console.log(`Total VT operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in VT`);
