...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main CA page to get all cities dynamically
    console.log('Fetching main CA page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/CA');
    ...
    console.log(`Found ${cityLinks.length} CA cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/CA\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/CA\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'CA' }, '-created_date', 2000);
    ...
    console.log(`Total CA operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in CA`);
