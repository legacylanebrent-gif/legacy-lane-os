...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main AZ page to get all cities dynamically
    console.log('Fetching main AZ page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/AZ');
    ...
    console.log(`Found ${cityLinks.length} AZ cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/AZ\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/AZ\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'AZ' }, '-created_date', 2000);
    ...
    console.log(`Total AZ operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in AZ`);
