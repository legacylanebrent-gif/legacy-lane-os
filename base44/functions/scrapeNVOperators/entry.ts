...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main NV page to get all cities dynamically
    console.log('Fetching main NV page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/NV');
    const mainPageHtml = await mainPageResponse.text();
    
    // Extract all city links
    const cityLinkRegex = /href="(\/companies\/NV\/[^"]+)"/g;
    ...
    console.log(`Found ${cityLinks.length} NV cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/NV\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/NV\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'NV' }, '-created_date', 2000);
    ...
    console.log(`Total NV Estate Sale Company Owners in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in NV`);
