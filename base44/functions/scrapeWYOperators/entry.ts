...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main WY page to get all cities dynamically
    console.log('Fetching main WY page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/WY');
    const mainPageHtml = await mainPageResponse.text();
    
    // Extract all city links
    const cityLinkRegex = /href="(\/companies\/WY\/[^"]+)"/g;
    ...
    console.log(`Found ${cityLinks.length} WY cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/WY\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/WY\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'WY' }, '-created_date', 2000);
    ...
    console.log(`Total WY operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in WY`);
