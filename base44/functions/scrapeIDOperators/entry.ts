...
Deno.serve(async (req) => {
  try {
        ...
        try {
          ...
          for (let i = 1; i < companyBlocks.length; i++) {
            ...
            try {
              ...
              if (name) {
                allCompanies.push({
                  company_name: name, city: cityFromUrl, state: 'ID', phone, website,
                  ...
                  source_url: companyUrl, source_state: 'ID'
                });
              }
            } catch (e) {}
          }
    ...
    let skip = 0;
    while (true) {
      const page = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'ID' }, '-created_date', 500, skip);
