...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch the main DE page to get all cities dynamically
    console.log('Fetching main DE page to extract cities...');
    const mainPageResponse = await fetch('https://www.estatesales.net/companies/DE');
    ...
    console.log(`Found ${cityLinks.length} DE cities to process`);
      ...
      try {
        ...
        for (let i = 1; i < companyBlocks.length; i++) {
          ...
          try {
            ...
            let nameMatch = block.match(/href="\/companies\/DE\/[^"]+\/\d{5}\/\d+"[^>]*>([^<]+)<\/a>/);
            if (!nameMatch) {
            ...
            // Extract URL
            const urlMatch = block.match(/href="(\/companies\/DE\/[^"]+\/\d{5}\/\d+)"/);
    ...
    const existing = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'DE' }, '-created_date', 2000);
    ...
    console.log(`Total DE operators in database: ${allOperators.length}`);
    ...
    console.log(`Found ${toDelete.length} duplicates in DE`);
