...
Deno.serve(async (req) => {
  try {
    ...
    if (cachedCompanies) {
      ...
      const mainPageResponse2 = await mainPageResponse.text();
      const cityLinkRegex = /href="(\/companies\/WV\/[^"]+)"/g;
      const cityLinks = [...mainPageResponse2.matchAll(cityLinkRegex)]
        ...
        try {
          ...
          for (let i = 1; i < companyBlocks.length; i++) {
            ...
            try {
              ...
              if (name) {
                allCompanies.push({
                  company_name: name, city: cityFromUrl, state: 'WV', phone, website,
                  ...
                  source_url: companyUrl, source_state: 'WV'
                });
              }
            } catch (e) {}
          }
    ...
    let skip = 0;
    while (true) {
      const page = await base44.asServiceRole.entities.FutureEstateOperator.filter({ state: 'WV' }, '-created_date', 500, skip);
