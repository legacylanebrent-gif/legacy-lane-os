...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`Found ${uniqueCityLinks.length} cities/regions in GA`);
    ...
    for (const cityUrl of uniqueCityLinks) {
      try {
        ...
        const companyLinkRegex = /href="(\/companies\/GA\/[^/]+\/\d{5}\/\d+)"/g;
        ...
        for (let j = 0; j < uniqueCompanyLinks.length; j += BATCH_SIZE) {
          ...
          const companyPromises = companyBatch.map(async (companyUrl) => {
            try {
              ...
              const urlParts = companyUrl.match(/\/companies\/GA\/([^/]+)\/(\d{5})\/\d+/);
              ...
              const stateCode = locationMatch ? locationMatch[2] : 'GA';
