...
Deno.serve(async (req) => {
  try {
    ...
    const stateUrl = 'https://www.estatesales.net/companies/DE';
    ...
    const cityLinkRegex = /href="(\/companies\/DE\/[^"<>]+?)(?:"|<)/g;
    ...
    console.log(`Found ${uniqueCityLinks.length} cities/regions in DE`);
    ...
    for (const cityUrl of uniqueCityLinks) {
      try {
        ...
        const companyLinkRegex = /href="(\/companies\/DE\/[^/]+\/\d{5}\/\d+)"/g;
        ...
        for (let j = 0; j < uniqueCompanyLinks.length; j += BATCH_SIZE) {
          ...
          const companyPromises = companyBatch.map(async (companyUrl) => {
            try {
              ...
              const urlParts = companyUrl.match(/\/companies\/DE\/([^/]+)\/(\d{5})\/\d+/);
              ...
              const stateCode = locationMatch ? locationMatch[2] : 'DE';
