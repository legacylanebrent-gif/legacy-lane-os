...
Deno.serve(async (req) => {
  try {
    ...
    const stateUrl = 'https://www.estatesales.net/companies/CT';
    console.log(`Fetching state page: ${stateUrl}`);
    
    const stateResponse = await fetch(stateUrl);
    const stateHtml = await stateResponse.text();
    
    // Extract city/region links
    const cityLinkRegex = /href="(\/companies\/CT\/[^"<>]+?)(?:"|<)/g;
    ...
    }).map(link => `https://www.estatesales.net${link}`);
    
    console.log(`Found ${uniqueCityLinks.length} cities/regions in CT`);
    ...
    for (const cityUrl of uniqueCityLinks) {
      try {
        ...
        const companyLinkRegex = /href="(\/companies\/CT\/[^/]+\/\d{5}\/\d+)"/g;
        const relativeCompanyLinks = [...cityHtml.matchAll(companyLinkRegex)].map(m => m[1]);
        const uniqueCompanyLinks = [...new Set(relativeCompanyLinks)].map(link => `https://www.estatesales.net${link}`);
        console.log(`  Found ${uniqueCompanyLinks.length} companies in ${cityUrl}`);
        ...
        for (let j = 0; j < uniqueCompanyLinks.length; j += BATCH_SIZE) {
          ...
          const companyPromises = companyBatch.map(async (companyUrl) => {
            try {
              ...
              const urlParts = companyUrl.match(/\/companies\/CT\/([^/]+)\/(\d{5})\/\d+/);
              ...
              const stateCode = locationMatch ? locationMatch[2] : 'CT';
