...
Deno.serve(async (req) => {
  try {
    ...
    const stateUrl = 'https://www.estatesales.net/companies/AR';
    ...
    const cityLinkRegex = /href="(\/companies\/AR\/[^"<>]+?)(?:"|<)/g;
    ...
    console.log(`Found ${uniqueCityLinks.length} cities/regions in AR`);
    ...
    for (const cityUrl of uniqueCityLinks) {
      try {
        ...
        const companyLinkRegex = /href="(\/companies\/AR\/[^/]+\/\d{5}\/\d+)"/g;
        ...
        for (let j = 0; j < uniqueCompanyLinks.length; j += BATCH_SIZE) {
          ...
          const companyPromises = companyBatch.map(async (companyUrl) => {
            try {
              ...
              const urlParts = companyUrl.match(/\/companies\/AR\/([^/]+)\/(\d{5})\/\d+/);
              ...
              const stateCode = locationMatch ? locationMatch[2] : 'AR';
