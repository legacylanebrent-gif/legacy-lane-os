...
import { Plus, Mail, Phone, MapPin, Clock, Users, AlertCircle, CheckCircle, User, TrendingUp, Facebook } from 'lucide-react';

const AD_PLATFORMS = ['Facebook', 'Instagram', 'TikTok', 'YouTube', 'Google'];
...
export default function AdminLeadsSocialAds() {
  ...
  const [form, setForm] = useState({
    ...
    property_address: '', situation: '', timeline: '',
    estimated_value: '', ad_platform: 'Facebook', ad_campaign: '',
    intent: 'estate_sale', score: 60, notes: ''
  });
  ...
  const loadData = async () => {
    ...
    const [leadsData, users] = await Promise.all([
      base44.entities.Lead.filter({ source: 'social_ads' }, '-created_date'),
  ...
  const handleAdd = async (e) => {
    ...
    setShowAdd(false);
    setForm({ contact_name: '', contact_email: '', contact_phone: '', property_address: '', situation: '', timeline: '', estimated_value: '', ad_platform: 'Facebook', ad_campaign: '', intent: 'estate_sale', score: 60, notes: '' });
    loadData();
  };
  ...
  const filtered = leads.filter(lead => {
    ...
    const match = !search || lead.contact_name?.toLowerCase().includes(q) || lead.contact_email?.toLowerCase().includes(q) || lead.property_address?.toLowerCase().includes(q);
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="p-5 flex items-center justify-between"><div><p className="text-sm text-slate-600 mb-1">Assigned</p><p className="text-3xl font-bold text-blue-600">{assigned}</p></div><User className="w-8 h-8 text-blue-200" /></CardContent></Card>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <Badge className="bg-blue-100 text-blue-700 text-xs">{lead.ad_platform || 'Social'}</Badge>
                    </div>
                  </div>
      ...
      <Dialog open={showDetail} onOpenChange={(open) => { setShowDetail(open); if (!open) setSelectedLead(null); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Social Ad Lead</DialogTitle></DialogHeader>
          {selectedLead && (
            <div className="space-y-4 mt-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-lg">{selectedLead.contact_name}</p>
                  <Badge className="bg-blue-100 text-blue-700">{selectedLead.ad_platform || 'Social'}</Badge>
                  {selectedLead.ad_campaign && <p className="text-xs text-slate-500 mt-1">Campaign: {selectedLead.ad_campaign}</p>}
                </div>
              </div>
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Add Social Ad Lead</DialogTitle></DialogHeader>
          <form onSubmit={handleAdd} className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>Ad Platform</Label>
                <Select value={form.ad_platform} onValueChange={v => setForm({ ...form, ad_platform: v })}>
                  ...
                  <SelectContent>{AD_PLATFORMS.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label>Ad Campaign Name</Label><Input value={form.ad_campaign} onChange={e => setForm({ ...form, ad_campaign: e.target.value })} placeholder="e.g. Spring Estate Sale Promo" /></div>
            ...
            <div className="flex gap-3 justify-end pt-4 border-t">
              ...
              <Button type="submit" className="bg-blue-600 hover:bg-blue-700">Add Lead</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
