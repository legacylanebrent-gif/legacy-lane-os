...
export default function AdminAIReportHistory({ reports, onView }) {
  ...
  return (
    <div className="space-y-3">
      {reports.map(report => (
        <div key={report.id} className="flex items-center justify-between gap-4 p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 hover:border-slate-600 transition-all">
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-slate-200 truncate">{report.title}</p>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              {report.command_type && (
                <span className="text-xs text-amber-400/70">{report.command_type}</span>
              )}
              {report.execution_mode && (
                <span className="text-xs text-slate-500">· {report.execution_mode}</span>
              )}
              {report.created_at && (
