...
export default function AISaleMarketingPackage({ sale, open, onClose, modelOverride, onSaved }) {
  ...
  const [compositorUrls, setCompositorUrls] = useState({});
...
## TONE: Confident, clear, action-driven, local-market aware. Everything tailored to THIS specific sale.`;
  ...
  const handleSaveCompositorImage = async (index, platformDataUrls) => {
    if (!sale) return;
    setSavingImageIndex(index);
    try {
      // Upload each platform version and collect URLs
      const uploadedUrls = {};
      await Promise.all(
        Object.entries(platformDataUrls).map(async ([platformKey, dataUrl]) => {
          const res = await fetch(dataUrl);
          const blob = await res.blob();
          const file = new File([blob], `post_${index + 1}_${platformKey}.jpg`, { type: 'image/jpeg' });
          const { file_url } = await base44.integrations.Core.UploadFile({ file });
          uploadedUrls[platformKey] = file_url;
        })
      );

      // Primary image = facebook_feed (best universal fallback)
      const primaryUrl = uploadedUrls.facebook_feed || Object.values(uploadedUrls)[0];

      // Store primary URL so text posts can also reference it
      setCompositorUrls(prev => ({ ...prev, [index]: primaryUrl }));

      // Sanitize string: remove surrogate characters that break UTF-8 encoding
      const sanitize = (str) => str ? str.replace(/[\uD800-\uDFFF]/g, '') : str;

      const existingId = savedPostIds[index];
      if (existingId) {
        await base44.entities.MarketingTask.update(existingId, {
          image_url: primaryUrl,
          image_urls_by_platform: uploadedUrls,
        });
      } else {
        const text = typeof result === 'string' ? result : JSON.stringify(result || '');
        const sections = parseSections(text);
        const boostSection = text.match(/(?:boost strategy|part 5)([\s\S]*?)(?:$)/i)?.[1] || '';
        const sectionText = sanitize(sections[index].join('\n').trim() || `Post ${index + 1}: AI-generated marketing post for ${sale.title}`);

        const created = await base44.entities.MarketingTask.create({
          ...
          image_url: primaryUrl,
          image_urls_by_platform: uploadedUrls,
          notes: sanitize(`AI-generated by ${modelLabel} | ${boostSection.slice(0, 400)}`),
        });
        setSavedPostIds(prev => ({ ...prev, [index]: created.id }));
  ...
  const handleSavePost = async (index) => {
    ...
    // Already saved via compositor — no duplicate
    if (savedPostIds[index]) {
      setSavedPosts(prev => ({ ...prev, [index]: true }));
      return;
    }
    setSaving(true);
    ...
    const boostSection = text.match(/(?:boost strategy|part 5)([\s\S]*?)(?:$)/i)?.[1] || '';
    const sectionText = sanitize(sections[index].join('\n').trim() || `Post ${index + 1}: AI-generated marketing post for ${sale.title}`);
    const imageUrl = compositorUrls[index] || null;
