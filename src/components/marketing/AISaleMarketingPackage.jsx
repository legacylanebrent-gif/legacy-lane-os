...
## TONE: Confident, clear, action-driven, local-market aware. Everything tailored to THIS specific sale.`;
  ...
  const handleSaveCompositorImage = async (index, dataUrl) => {
    ...
    try {
      ...
      // Store the uploaded URL so text posts can also reference it
      setCompositorUrls(prev => ({ ...prev, [index]: file_url }));

      // If text post already saved, update it with the image instead of creating duplicate
      const existingId = savedPostIds[index];
      if (existingId) {
        await base44.entities.MarketingTask.update(existingId, { image_url: file_url });
      } else {
